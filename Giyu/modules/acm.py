...
from Giyu.modules.sql import acm_sql
from Giyu import THUNDERS, dispatcher, TOKEN
...
from Water.GiyuStrings.Estrings import acm, acm0, acm1, acm2, acm3
...
from Giyu.modules.sql import acm_sql
from Giyu.modules.helper_funcs.decorators import GiyuCMD 
...
def cleanlinked(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if param == "on" or param == "true" or param == "yes" or param == "On" or param == "Yes" or param == "True":
            ...
            msg.reply_text(acm.format(chat.title), parse_mode=ParseMode.MARKDOWN)
            return
        elif param == "off" or param == "false" or param == "no" or param == "No" or param == "Off" or param == "False":
            acm_sql.setCleanLinked(chat.id, False)
            msg.reply_text(acm0.format(chat.title), parse_mode=ParseMode.MARKDOWN)
            return
        else:
            msg.reply_text(acm1) #on or off ffs
            return
    else:
        stat = acm_sql.getCleanLinked(str(chat.id))
        if stat:
            msg.reply_text(acm2.format(chat.title), parse_mode=ParseMode.MARKDOWN)
            return
        else:
            msg.reply_text(acm3.format(chat.title), parse_mode=ParseMode.MARKDOWN)
            return
...
CLEANLINKED_HANDLER = CommandHandler(['acm', 'antichannel'], cleanlinked, filters=Filters.chat_type.groups, run_async=True)
