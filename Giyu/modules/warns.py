...
import telegram
from Giyu import WATERS, BEASTS, dispatcher
...
from Giyu.modules.helper_funcs.misc import split_message
from Giyu.modules.helper_funcs.string_handling import split_quotes
from Giyu.modules.log_channel import loggable
from Giyu.modules.sql import warns_sql as sql
...
from Giyu.modules.sql.approve_sql import is_approved
...
def warn(user: User,
    ...
    if user.id in WATERS:
        if warner:
            message.reply_text("Water Breathers cant be warned.")
        else:
            message.reply_text(
                "Water Breather triggered an auto warn filter!\n I can't warn Water Breathers but they should avoid abusing this."
            )
        return

    if user.id in BEASTS:
        if warner:
            message.reply_text("Beast Breathers are warn immune.")
        else:
            message.reply_text(
                "Beast Breathers triggered an auto warn filter!\nI can't warn Beast Breathers but they should avoid abusing this."
            )
        return
    ...
    if args:
        if args[0].lower() in ("on", "yes"):
            ...
            return (
                f"<b>{html.escape(chat.title)}:</b>\n"
                f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
                f"Has disabled kicks, I will use normal kicks on users."
            )
