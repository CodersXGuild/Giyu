...
from Giyu.modules.sql import SESSION, BASE
...
class Permissions(BASE):
    ...
    text_mention = Column(Boolean, default=False)
...
def update_lock(chat_id, lock_type, locked):
    with PERM_LOCK:
        ...
        elif lock_type == "text_mention":
            curr_perm.text_mention = locked 
        elif lock_type == "phone_number":
            curr_perm.phone_number = locked 
...
def is_locked(chat_id, lock_type):
    ...
    if lock_type == "text_mention":
        return curr_perm.text_mention 
    if lock_type == "phone_number":
        return curr_perm.phone_number
    if lock_type == "email":
