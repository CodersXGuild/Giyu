...
import Giyu.modules.sql.welcome_sql as sql 
from Giyu import (
    OWNER_USERNAME,
    CO_OWNER_USERNAME,
    )
...
from Giyu.modules.helper_funcs.misc import build_keyboard, revert_buttons
from Giyu.modules.helper_funcs.msg_types import get_welcome_type
from Giyu.modules.helper_funcs.handlers import MessageHandlerChecker
from Giyu.modules.helper_funcs.string_handling import (
...
WHITELISTED = OWNER_ID, CO_OWNER_ID, FLAMES, THUNDERS, WINDS, WATERS, BEASTS
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text(
                    f"Welcome to {html.escape(chat.title)} @{OWNER_USERNAME}.", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Sun Breather Arrived."
                )
                continue 
            
            if new_mem.id == CO_OWNER_ID:
                update.effective_message.reply_text(
                    f"Welcome Moon Breather @{CO_OWNER_USERNAME}-sama OwO.", reply_to_message_id=reply 
                ) 
                welcome_log = ( 
                    f"{html.escape(chat.title)}\n" 
                    f"#USER_JOINED\n"
                    f"Moon Breather Arrived."  
                )
                continue
                    

            # Welcome Devs
            if new_mem.id in FLAMES:
                update.effective_message.reply_text(
                    "Whoa! A Flame Breather just joined OwO!",
                    reply_to_message_id=reply,
                )
                continue

            # Welcome Sudos
            if new_mem.id in THUNDERS:
                update.effective_message.reply_text(
                    "Huh! Thunder Breather!",
                    reply_to_message_id=reply,
                )
                continue

            # Welcome Support
            if new_mem.id in WINDS:
                update.effective_message.reply_text(
                    "Wind Breather just joined",
                    reply_to_message_id=reply,
                )
                continue 
            # Welcome TIGRERS 
            if new_mem.id in WATERS: 
                update.effective_message.reply_text(
                    "Heyoo!! Water Breather",
                    reply_to_message_id=reply,
                ) 
                continue

            # Welcome WOLVES
            if new_mem.id in BEASTS:
                update.effective_message.reply_text(
                    "Yo Beast Breather!", reply_to_message_id=reply
                )
                continue
...
def left_member(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id in FLAMES:
                update.effective_message.reply_text(
                    "Sayonara Flame Breather Sama!",
                    reply_to_message_id=reply,
                )
                return
...
def welcome_help(update: Update, context: CallbackContext):
    chat = update.effective_chat
    update.effective_message.reply_text(
        WELCOME_HELP_TEXT, parse_mode=ParseMode.MARKDOWN
    )
...
def welcome_mute_help(update: Update, context: CallbackContext):
    chat = update.effective_chat
    update.effective_message.reply_text(
        WELCOME_MUTE_HELP_TEXT, parse_mode=ParseMode.MARKDOWN
    )
