...
import Giyu.modules.sql.notes_sql as sql
from Giyu import dispatcher, LOGGER, OWNER_ID, CO_OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from Giyu.__main__ import DATA_IMPORT
...
# from Giyu.modules.rules import get_rules
import Giyu.modules.sql.rules_sql as rulessql

# from Giyu.modules.sql import warns_sql as warnssql
import Giyu.modules.sql.blacklist_sql as blacklistsql
from Giyu.modules.sql import disable_sql as disabledsql

# from Giyu.modules.sql import cust_filters_sql as filtersql
# import Giyu.modules.sql.welcome_sql as welcsql
import Giyu.modules.sql.locks_sql as locksql
from Giyu.modules.connection import connected
...
def import_data(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            update.effective_message.reply_text(lynn.bck1)
            return ""
    ...
    if msg.reply_to_message and msg.reply_to_message.document:
        ...
        try:
            ...
            return msg.reply_text(lynn,bck6)
        # Check if backup is from self
        try:
            if str(context.bot.id) != str(data[str(chat.id)]["bot"]):
                return msg.reply_text(lynn.bck7)
        except Exception:
            pass
        # Select data source
        ...
        if conn:
            ...
            text = lynn.bck8.format(chat_name)
        else:
            text = lynn.bck9
...
def export_data(update, context):
    ...
    with open("Giyu{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Giyu{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `GiyuKamado-Backup` was specially made for notes.".format(
            chat.title,
            chat_id,
            tgl,
        ),
    ...
    os.remove("Giyu{}.backup".format(chat_id))  # Cleaning file
