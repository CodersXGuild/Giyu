...
from Giyu import FLAMES, THUNDERS, dispatcher
from .decorators import GiyuCALLBACK
...
def userr_admin(permission: AdminPerms):
    def wrapper(func):
        ...
        def awrapper(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            if is_anon:
                ...
                if getattr(mem, permission.value) is True or mem.status == "creator" or user_id in THUNDERS:
                    return func(update, context, *args, **kwargs)
                else:
...
def anon_callback_handler1(upd: Update, _: CallbackContext):
    ...
    elif getattr(mem, perm) is True or mem.status == "creator" or mem.user.id in FLAMES:
