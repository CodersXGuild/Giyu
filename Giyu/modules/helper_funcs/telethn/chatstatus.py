from Giyu.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from Giyu import THUNDERS
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in THUNDERS:
            status = True
            break
    return status
...
async def Giyu_is_admin(chat_id: int):
    status = False
