...
from Giyu import THUNDERS, FLAMES, dispatcher
    ...
    def wrapper(func):
        ...
        def awrapper(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            if is_anon:
                ...
                if getattr(mem, permission.value) is True or mem.status == "creator" or user_id in FLAMES:
                    return func(update, context, *args, **kwargs)
                else:
...
def anon_callback_handler1(upd: Update, _: CallbackContext):
    ...
    elif getattr(mem, perm) is True or mem.status == "creator" or mem.user.id in THUNDERS:
